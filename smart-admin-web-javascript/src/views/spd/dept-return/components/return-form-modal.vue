<template>
  ...
  >
    <a-form :model="form" :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }" ref="formRef">
      <a-row :gutter="16">
        <a-col :span="12">
          <a-form-item label="退库科室" name="fromNodeId" :rules="[{ required: true, message: '请选择退库科室' }]">
            <a-input v-model:value="form.fromNodeId" placeholder="请输入退库科室节点ID" />
          </a-form-item>
        </a-col>
        <a-col :span="12">
          <a-form-item label="接收仓库" name="toNodeId" :rules="[{ required: true, message: '请选择接收仓库' }]">
            <a-input v-model:value="form.toNodeId" placeholder="请输入接收仓库节点ID" />
          </a-form-item>
        </a-col>
      </a-row>
      ...
      <a-row :gutter="16">
        <a-col :span="12">
          <a-form-item label="退库类型" name="returnType" :rules="[{ required: true, message: '请选择退库类型' }]">
            <a-select v-model:value="form.returnType" placeholder="请选择退库类型">
      ...
      <a-table :dataSource="form.detailList" :columns="detailColumns" :pagination="false" rowKey="index" size="small" bordered>
        <template #bodyCell="{ text, record, column, index }">
          ...
          <template v-if="column.dataIndex === 'returnReason'">
            <a-input v-model:value="record.returnReason" placeholder="退库原因" />
          </template>
  ...
  async function handleSubmit() {
    try {
      ...
      if (form.detailList.length === 0) {
        message.warning('请至少添加一条退库明细');
        return;
      }
      
      const loading = SmartLoading.show();
      try {
        await deptReturnApi.add(form);
