<template>
  ...
  >
    <a-form :model="form" :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }" ref="formRef">
      <a-row :gutter="16">
        <a-col :span="12">
          <a-form-item label="调出节点" name="fromNodeId" :rules="[{ required: true, message: '请选择调出节点' }]">
            <a-input v-model:value="form.fromNodeId" placeholder="请输入调出节点ID" />
          </a-form-item>
        </a-col>
        <a-col :span="12">
          <a-form-item label="调入节点" name="toNodeId" :rules="[{ required: true, message: '请选择调入节点' }]">
            <a-input v-model:value="form.toNodeId" placeholder="请输入调入节点ID" />
          </a-form-item>
        </a-col>
      </a-row>
      ...
      <a-table :dataSource="form.detailList" :columns="detailColumns" :pagination="false" rowKey="index" size="small" bordered>
        <template #bodyCell="{ text, record, column, index }">
          ...
          <template v-if="column.dataIndex === 'allocationQuantity'">
            <a-input-number v-model:value="record.allocationQuantity" :min="1" style="width: 100%" />
          </template>
...
<script setup>
  ...
  const detailColumns = ref([
    { title: '耗材ID', dataIndex: 'materialId', width: 200 },
    { title: '批号ID', dataIndex: 'batchId', width: 200 },
    { title: '调拨数量', dataIndex: 'allocationQuantity', width: 120 },
